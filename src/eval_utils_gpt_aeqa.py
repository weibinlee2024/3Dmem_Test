...
# client = OpenAI(
#     base_url=QWEN_VL_BASE_URL,  # <-- 使用 Qwen-VL 的 Base URL
#     api_key=QWEN_VL_API_KEY,    # <-- 使用 Qwen-VL 的 API Key
#   )
# client = OpenAI(
#       base_url=END_POINT,
#       api_key=OPENAI_KEY,
...
client = OpenAI(
    base_url=INTERNVL_BASE_URL,  # <-- 使用 InternVL2_5-8B 的 Base URL
    api_key=INTERNVL_API_KEY,    # <-- 使用 InternVL2_5-8B 的 API Key
  )
    ...
    while retry_count < max_tries:
        try:
            completion = client.chat.completions.create(
                ...
                #model="/home/ubuntu/projects2/Qwen2.5-VL/Qwen/Qwen2.5-VL-7B-Instruct",
